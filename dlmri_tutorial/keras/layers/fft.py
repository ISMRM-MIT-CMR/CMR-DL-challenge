...
class IFFT2c(tf.keras.layers.Layer):
    def call(self, kspace, *args):
        ...
        return dlmri_tutorial.complex_scale(fftshift(ifft2d(ifftshift(kspace, axes=axes)), axes=axes), scale)
...
class FFT2c(tf.keras.layers.Layer):
    def call(self, image, *args):
        ...
        return dlmri_tutorial.complex_scale(fftshift(fft2d(ifftshift(image, axes=axes)), axes=axes), 1/scale)
